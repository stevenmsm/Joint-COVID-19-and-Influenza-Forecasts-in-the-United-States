...
current_dic = "~/Documents/Georgia_Tech/Research at GATECH/Research with Dr. Shihao Yang/COVID-19/FLU+COVID19/"
...
ili_version <- "ili20210920/"
ili.folder <- paste0(current_dic, ili_version)
population.file <- "Population.csv"
#load(file.path(ili.folder, "results_44b_210707_gft_GTapi_raw.Rdata"))
GTpub_all <- "api_raw_results-2021-09-12"
GTpub_version <- paste0(GTpub_all, "/")
out.folder <- paste0(ili.folder, GTpub_version)

## read in first step flu predictions
reg.weight.mix <- 1/3
idx.GTregmix <- F
idx.GTstate <- T ## argox individual step 2 with state-only GT
idx.lagstate <- NULL
...
load(file.path(out.folder, "data_reg_all_logit01_lag0a.Rdata"))
file.name <- paste0("data_states_all_logit01_", gt.out, "a.Rdata")
load(file.path(out.folder, file.name))


## state data
states.abb.all <- colnames(ili_state)
tab.states <- read.csv("./Population.csv")
tab.states$Population <- as.numeric(gsub(",", "", tab.states$Population))

## add in NYC data for flu prediction
df_nyc <- data.frame(State="New York City", Abbre="NYC", Population= 8175133, Region = 2)
tab.states <- rbind(tab.states, df_nyc)

tab.states$Abbre_names <- sapply(tab.states$Abbre, function(x){paste0("US-", x)})
tab.states$Abbre_names_flu <- sapply(tab.states$Abbre, function(x){paste0("US.", x)})


## whether to read-in/process COVID death/case data
idx_death <- T
idx_cases <- T

## read in COVID data
covid_folder <- paste0(ili.folder, "covid")
load(file.path("./", "0912_Step1_Final.rda"))  

covid_nowcast <- list_results_State_FINAL$`1 week ahead`
covid_nowcast_nat <- list_results_Nat_FINAL$`1 week ahead`
covid_nowcast_reg <- list()


### covid cases NYT
## national
tab_covid_case_national <- read.csv(file.path('./', "NYT_us_national.csv"))
covid_case_national_daily <- xts(tab_covid_case_national$cases, order.by = as.Date(tab_covid_case_national$date))
covid_case_national_incre <- diff(covid_case_national_daily)

## weekly increment of cases
temp = na.omit(covid_case_national_incre)
index(temp) = index(temp) +1
temp = apply.weekly(temp,colSums) 
temp = xts(temp, as.Date(index(temp)))  
index(temp) = index(temp) - 8 #Now making index Saturdays. i.e. 5/2 store 5/3+...+5/9, consistent with covid pred
covid_case_nat_weekly_incre = temp
colnames(covid_case_nat_weekly_incre) <- "cases"

tab_covid_case_state <- read.csv(file.path("./", "NYT_us_states.csv"))
...
#write.csv(tt1, "ARGOX_ind_covid_cases_curr_inreg_flureg_w0.1_MSE_raw_percent_train24.csv")
