...
save(covid_national_level, covid_state_level, gtdataALL_IQR_noMedia, gtdata_Region_IQR_noMedia, gtdata_stateMixed_IQR_noMedia, file="Data_0814.rda")

#*************************************************************************************************************************************
gtdata_national = gtdata_daily_state[["US"]]
ratio_term = length(names(gtdata_daily_state[[regions]]))
...
for (terms in terms_important){
  ...
  temp_date =  as.Date(as.Date("2020-04-01"):as.Date("2020-07-01"))
...
if (if_use_flu){
  cases_nat_step1_withFlu_08142022 = list()
  for (i in 1:4){
    pred_week = ((i-1)*7+1):(i*7)
    week_ahead = paste0(i, " week ahead")
    cases_nat_step1_withFlu_08142022[[week_ahead]] = lapply(pred_week, function(x) list_results_step1.Nat[[x]])
    cases_nat_step1_withFlu_08142022[[week_ahead]] = Reduce("+", cases_nat_step1_withFlu_08142022[[week_ahead]]) 
  }
}

cases_nat_step1_noFlu_08142022 = list()
for (i in 1:4){
  pred_week = ((i-1)*7+1):(i*7)
  week_ahead = paste0(i, " week ahead")
  cases_nat_step1_noFlu_08142022[[week_ahead]] = lapply(pred_week, function(x) list_results_step1.Nat[[x]])
  cases_nat_step1_noFlu_08142022[[week_ahead]] = Reduce("+", cases_nat_step1_noFlu_08142022[[week_ahead]]) 
}
...
for (i in 1:4){
  ...
  for(i in names(list_results_step1.Reg)){  
    ...
    cases_reg_step1_08142022[[week_ahead]][[i]] = results_week.s
  }
}
...
cases_state_step1_08142022 = list()
for (i in 1:4){
  pred_week = ((i-1)*7+1):(i*7)
  week_ahead = paste0(i, " week ahead")
  for(i in names(list_results_step1.State)){  
    ...
    cases_state_step1_08142022[[week_ahead]][[i]] = results_week.s
  }
}
...
list_results_Reg_FINAL[[weeks_ahead]] = cases_reg_step1_08142022[[weeks_ahead]]
list_results_State_FINAL = list()
list_results_State_FINAL[[weeks_ahead]] = cases_state_step1_08142022[[weeks_ahead]]
list_results_Nat_FINAL = list()
list_results_Nat_FINAL[[weeks_ahead]] = cases_nat_step1_noFlu_08142022[[weeks_ahead]]
if_no_temp_region_state = TRUE

if (if_no_temp_region_state){
  temp_region = list()
...
training_period = 120  #COVID-19 use 70 days, 10 weeks training period
...
for (t.now in index(na.omit(err.twostep_ALL))){
  ...
  argo2_Ensumble.p[date_idx,] = apply((argo_pred_now[date_idx-7*weeks_ahead,] -   matrix(rep(t(truth_ALL[date_idx-7*weeks_ahead,]),each =3),nrow=3))^2, 2, min) + truth_ALL[date_idx, ]
}
...
weight_diag = 0.2 # Weight on the Variance and Diagonal Matrix Linear Combo
