...
if (FALSE){ # Fit one linear model use all data find the best lag and see if they are significant w.r.p to the sandwich estimated p-val
  ...
  for (terms in terms_important){
    ...
    temp_date =  as.Date(as.Date("2021-04-01"):as.Date("2021-12-31"))
...
if (FALSE){
  load("~/Documents/Georgia_Tech/Research at GATECH/Research with Dr. Shihao Yang/COVID-19/ARGOX/All_Processed_Data_20211010.rda")
...
case_lag <- c(7,14,21,28,35) #a week and two weeks
# flu_lag <- c(14,21,28)
flu_lag <- c(7,14,21,28)
...
if (FALSE){
  list_results_Nat_0328 = list()
  pred_week = 1:7
  list_results_Nat_0328[["1 week ahead"]] = lapply(pred_week, function(x) list_results_step1.Nat[[x]])
  list_results_Nat_0328[["1 week ahead"]] = Reduce("+", list_results_Nat_0328[["1 week ahead"]]) 
  #list_results_Nat_0328[["1 week ahead"]] = list_results_Nat_0328[["1 week ahead"]]["2021-01/",]
  plot(na.omit(list_results_Nat_0328[["1 week ahead"]]["2020-05/",-2]),legend.loc = "topleft")
  pred_week = 8:14
  list_results_Nat_0328[["2 week ahead"]] = lapply(pred_week, function(x) list_results_step1.Nat[[x]])
  list_results_Nat_0328[["2 week ahead"]] = Reduce("+", list_results_Nat_0328[["2 week ahead"]]) 
  #list_results_Nat_0328[["2 week ahead"]] = list_results_Nat_0328[["2 week ahead"]]["2021-01/",]
  plot(na.omit(list_results_Nat_0328[["2 week ahead"]]["2020-05/",-2]),legend.loc = "topleft")
  pred_week = 15:21
  list_results_Nat_0328[["3 week ahead"]] = lapply(pred_week, function(x) list_results_step1.Nat[[x]])
  list_results_Nat_0328[["3 week ahead"]] = Reduce("+", list_results_Nat_0328[["3 week ahead"]]) 
  plot(na.omit(list_results_Nat_0328[["3 week ahead"]]["2020-05/",-2]),legend.loc = "topleft")
  pred_week = 22:28
  list_results_Nat_0328[["4 week ahead"]] = lapply(pred_week, function(x) list_results_step1.Nat[[x]])
  list_results_Nat_0328[["4 week ahead"]] = Reduce("+", list_results_Nat_0328[["4 week ahead"]]) 
  #list_results_Nat_0328[["2 week ahead"]] = list_results_Nat_0328[["2 week ahead"]]["2021-01/",]
  plot(na.omit(list_results_Nat_0328[["4 week ahead"]]["2020-05/",-2]),legend.loc = "topleft")
  coef_all_Nat_0328 =  coef_all_step1.Nat
  coef_smooth_Nat_0328  = coef_smooth_step1.Nat
}
if (FALSE){
...
total_forward = 7  #predicting how many days forward
decay <- 0.8
...
if (FALSE){
  ...
  pred_week = 8:14
  for (i in names(list_results_step1.Reg)){
    list_results_Reg_0328[["2 week ahead"]][[i]]  = lapply(pred_week, function(x) list_results_step1.Reg[[i]][[x]])
    list_results_Reg_0328[["2 week ahead"]][[i]]  = Reduce("+", list_results_Reg_0328[["2 week ahead"]][[i]] ) 
    #list_results_Reg_0328[["2 week ahead"]][[i]] = list_results_Reg_0328[["2 week ahead"]][[i]]["2021-01/",]
  }
  coef_all_Reg_0328 =  coef_all_step1.Reg
  coef_smooth_Reg_0328  = coef_smooth_step1.Reg
  ...
  temp_region = list()
  weeks_ahead = 1
  for(i in names(list_results_Reg_0328[[weeks_ahead]])){  
    #temp_region[[i]] = na.omit(list_results_Reg_0328[[weeks_ahead]][[i]][-c(1:(which(index(list_results_Reg_0328[[weeks_ahead]][[i]])=="2020-05-01")-1)),])
    temp_region[[i]] = na.omit(list_results_Reg_0328_2021Pred[[weeks_ahead]][[i]])
  }
  temp_region<- lapply(names(list_results_Reg_0328[[weeks_ahead]]), function(x){apply((sweep(temp_region[[x]], 1,temp_region[[x]]$truth, "-"))^2, 2, mean)})
  temp_region = do.call(cbind,temp_region)
  colnames(temp_region) = names(list_results_Reg_0328[[weeks_ahead]])
}
if (FALSE){
  pred_week = 1:7
  for(i in names(list_results_step1.Reg)){  
  ...
  temp_region = list()
  for(i in names(list_results_step1.Reg)){  
    temp_region[[i]] = list_results_step1.Reg[[i]][-c(1:(which(index(list_results_step1.Reg[[i]])=="2021-03-28")-1)),]
  }
...
dif (FALSE){
  ...
  pred_week = 8:14
  for (i in names(list_results_step1.State)){
    list_results_State_0328[["2 week ahead"]][[i]]  = lapply(pred_week, function(x) list_results_step1.State[[i]][[x]])
    list_results_State_0328[["2 week ahead"]][[i]]  = Reduce("+", list_results_State_0328[["2 week ahead"]][[i]] ) 
    #list_results_State_0328_2021Pred[["2 week ahead"]][[i]] = list_results_State_0328_2021Pred[["2 week ahead"]][[i]]["2021-01/",]
  }
  coef_all_State_0328 =  coef_all_step1.State
  coef_smooth_State_0328  = coef_smooth_step1.State
  ...
  temp_state = list()
  weeks_ahead = 1
  for(i in names(list_results_State_0328[[weeks_ahead]])){  
    #temp_state[[i]] = na.omit(list_results_State_0328[[weeks_ahead]][[i]][-c(1:(which(index(list_results_State_0328[[weeks_ahead]][[i]])=="2020-05-01")-1)),])
    temp_state[[i]] = na.omit(list_results_State_0328_2021Pred[[weeks_ahead]][[i]])
  }
  temp_state<- lapply(names(list_results_State_0328[[weeks_ahead]]), function(x){apply((sweep(temp_state[[x]], 1,temp_state[[x]]$truth, "-"))^2, 2, mean)})
  temp_state = do.call(cbind,temp_state)
  colnames(temp_state) = names(list_results_State_0328[[weeks_ahead]])
}
if (FALSE){
  pred_week = 1:7
  for(i in names(list_results_step1.State)){  
